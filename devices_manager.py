...
class DevicesManager():
    def __init__(self, hwnd) -> None:
        ...
        self.hwnd = hwnd
        winrawin.hook_raw_input_for_window(hwnd, self._handle_event, device_types = ('Pointer', 'Mouse', 'Keyboard', 'Keypad'))
    ...
    def _handle_event(self, e: winrawin.RawInputEvent):
        if(e.device_type in ['mouse','pointer']):
            if e.event_type == 'move':
                ...
                print(f"Released {e.name} on {e.device_type} {e.device.handle}")
    ...
    def master_loop_iter(self):
        ...
        if(len(self.mouse_callbacks) and 
            ...
            for cb in self.mouse_callbacks:
                cb({"cmd":"move", "vpos":self.virtual_pos, "monitor":self.inside, "monitor_pos":monitor_pos})
